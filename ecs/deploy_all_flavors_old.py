...
#        print("server_summ:", server_summation)
        ...
        for index, summation in enumerate(summations):
            result += 100000 * summation / server_summation[index] 
#        print("result:", result)
        return result
...
#        evaluate_individual(mutated_group, )
    ...
    while(time.clock()-start_time < duration_time):
        ...
        print(evaluation[0][0]/float(2000), len(allocate_server(evaluation[0][1], server_limitation, flavor_specification)[1]))
...
def gen_predict_res(flavor_names,flavor_prediction):
    res=''
    sum_of_flavors=0
...
def deploy_flavors(flavor_names,flavor_prediction,flavor_type,server_cfg,serverNames):
    ...
    for index,count in enumerate(server_count):
        ...
        for index_alloc,alloc in enumerate(res_alloc):
            ...
            server_i_count+=1
        res+='\n'
    res_predict=gen_predict_res(flavor_names,flavor_prediction)
    res=res_predict+'\n'+res
    return res
